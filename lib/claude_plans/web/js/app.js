...
function applyTheme(t) {
  ...
  const moonSvg = '<svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M12 3a6 6 0 0 0 9 9 9 9 0 1 1-9-9Z"/></svg>';
...
Hooks.PlanContent = {
  mounted() { this.el.__liveViewHook = this; this._matchMarks = []; this._matchIdx = -1; this.render(); this.setupInspector(); },
  updated() { this.render(); },
  async render() {
    if (typeof mermaid !== 'undefined') {
  ...
  highlightSearch() {
    ...
    // Determine which mark to focus: data-highlight-line = match group index within document
    let startIdx = 0;
    const matchInDoc = this.el.dataset.highlightLine;
    if (matchInDoc != null && matchInDoc !== "") {
      const targetGroupIdx = parseInt(matchInDoc);
      if (!isNaN(targetGroupIdx) && targetGroupIdx > 0 && allMarks.length > 1) {
        // Group marks by parent block element
        const BLOCKS = ['P','LI','H1','H2','H3','H4','H5','H6','PRE','TR','TD','TH','BLOCKQUOTE','DT','DD'];
        const self = this;
        function getBlock(mark) {
          let node = mark.parentElement;
          while (node && node !== self.el) {
        ...
        let groupIdx = 0;
        let prevBlock = getBlock(allMarks[0]);
        for (let i = 1; i < allMarks.length; i++) {
          const currBlock = getBlock(allMarks[i]);
        ...
        if (groupIdx < targetGroupIdx) startIdx = allMarks.length - 1;
      }
    }

    this._matchIdx = startIdx;
    allMarks[startIdx].classList.add("cb-content-highlight--current");
    setTimeout(() => allMarks[startIdx].scrollIntoView({ behavior: "smooth", block: "center" }), 100);
    ...
    if (wrap) {
      ...
      countSpan.textContent = (startIdx + 1) + " / " + allMarks.length;
  ...
  setupInspector() {
    ...
    document.addEventListener("mousedown", function(e) {
      ...
      // pushEventTo routes based on element ownership:
      // Plans tab → BrowserLive, Folders tab → FoldersViewerComponent
      self.pushEventTo(self.el, "add_annotation", { block_path: blockPath, block_index: blockIndex });
    }, true);
  ...
  _computeBlockPath(el, clickEvent, mermaidPart) {
    let heading = null;
    // If the clicked element itself is a heading, use it directly
    if (/^H[1-6]$/.test(el.tagName)) {
      heading = el;
    }
    let node = el;
    while (!heading && node && node !== this.el) {
    ...
    if (/^h[1-6]$/.test(tag)) {
      ...
      const preview = liText.length > 40 ? liText.substring(0, 40) + "..." : liText;
      typeLabel = "bullet " + pos + (preview ? ': "' + preview + '"' : "");
    } else if (tag === "pre") {
      ...
      const pPreview = pText.length > 40 ? pText.substring(0, 40) + "..." : pText;
      typeLabel = (count > 1 ? "paragraph " + count : "paragraph") + (pPreview ? ': "' + pPreview + '"' : "");
      ...
      let sib = el;
      while (sib) {
        if (sib === heading) break;
        if (sib.classList && sib.classList.contains("mermaid")) diagramCount++;
        sib = sib.previousElementSibling;
      }
      typeLabel = diagramCount > 1 ? "diagram " + diagramCount : "diagram";
      if (mermaidPart) {
  ...
  inlineMermexSvgs() {
    const imgs = this.el.querySelectorAll(".mdex-mermex img[src^='data:image/svg+xml;base64,']");
    for (const img of imgs) {
      try {
        const b64 = img.src.replace("data:image/svg+xml;base64,", "");
        ...
        if (container) this.tagMermaidElements(container);
      } catch (e) {
        console.error("Failed to inline mermex SVG:", e);
      }
    }
  },
  setupEdgeHitTargets() {
...
Hooks.KeyboardNav = {
  mounted() {
    ...
    this.handleKeyDown = (e) => {
      ...
      const planContentEl = document.getElementById('plan-content') || document.getElementById('project-file-content') || document.getElementById('folder-file-content');
      ...
      switch(e.key) {
        ...
        case '2':
          ...
          break;
        case '3':
          this.pushEvent("kb_tab", {tab: "folders"});
          e.preventDefault();
          break;
        case '4':
          ...
          break;
        case 'n':
          this.pushEvent("kb_next_match", {});
          e.preventDefault();
          break;
        case 'N':
          this.pushEvent("kb_prev_match", {});
          e.preventDefault();
          break;
        case ']':
...
let liveSocket = new window.LiveView.LiveSocket("/live", window.Phoenix.Socket, {
  params: () => {
    const prefs = loadPrefs();
    return { _csrf_token: csrfToken, font_size: prefs.font_size || 16, content_width: prefs.content_width || "wide" };
  },
  hooks: Hooks
});
liveSocket.connect();
